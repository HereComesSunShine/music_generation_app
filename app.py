...
network_input_classic, normalized_input_classic = classic.prepare_sequences_output(notes_classic, pitchnames_classic, n_vocab_classic)
network_input_irish, normalized_input_irish = irish.prepare_sequences_output(notes_irish, pitchnames_irish, n_vocab_irish)        
network_input_combo, normalized_input_combo = combo.prepare_sequences_output(notes_combo, pitchnames_combo, n_vocab_combo)
...
def generate_music():
    ...
    if 'midiFile' not in request.files:
        ...
        if midi_file.filename == '' or midi_file.filename == 'No file chosen':
            ...
            midi_file.save(os.path.join(app.config['UPLOAD_FOLDER'], midi_file.filename))
    ...
    if style == 'classical':
        ...
        if not no_file:
            ...
            notes = parser.get_notes(os.path.join(app.config['UPLOAD_FOLDER'], midi_file.filename))
            try:
                print('Prepearing sequences...')
                wrong_pitch = create_seq.check_pitches(notes, pitchnames_classic)
                if wrong_pitch:
                    print('Wrong pitch')
                    notes, success_transform = create_seq.transform_pitch(notes, pitchnames_classic)
                    
                
                network_input_classic, normalized_input_classic = create_seq.prepare_sequences_output(notes, pitchnames_classic, n_vocab_classic)
            except Exception: 
                return abort(400, {'error': 'Cannot generate based on the uploaded file.'})
        else:
            print('Prepearing sequences...')
            network_input_classic, normalized_input_classic = create_seq.prepare_sequences_output(notes_classic, pitchnames_classic, n_vocab_classic)
        
        try:
           
            prediction_output = classic.generate_notes(model_classic,network_input_classic, pitchnames_classic, n_vocab_classic, random, int(composition_length))
        
            gen_midi.create_midi(prediction_output,instrument,generated_filename,GENERATED_FOLDER)
        except Exception: 
            return abort(400, {'error': 'Cannot generate based on the uploaded file.'})
        ...
        if not no_file:
            ...
            network_input_irish, normalized_input_irish = create_seq.prepare_sequences_output(notes, pitchnames_irish, n_vocab_irish)
        else:
            print('Prepearing sequences...')
            
            network_input_irish, normalized_input_irish = create_seq.prepare_sequences_output(notes_irish, pitchnames_irish, n_vocab_irish)
        
        
        
        prediction_output = irish.generate_notes(model_irish,network_input_irish, pitchnames_irish, n_vocab_irish, random, int(composition_length))
        gen_midi.create_midi(prediction_output,instrument,generated_filename,GENERATED_FOLDER)
        ...
        if not no_file:
            ...
            try:
                ...
                network_input_combo, normalized_input_combo = create_seq.prepare_sequences_output(notes, pitchnames_combo, n_vocab_combo)
            except Exception: 
                return abort(400, {'error': 'Cannot generate based on the uploaded file.'})
        else:
            print('Prepearing sequences...')
        ...
        try:
            ...
            prediction_output = combo.generate_notes(model_combo,network_input_combo, pitchnames_combo, n_vocab_combo, random, int(composition_length))
